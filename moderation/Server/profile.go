...
func EditProfileGet(w http.ResponseWriter, r *http.Request, cookie *http.Cookie, user *structs.User) {
	tmpl, tmplErr := template.ParseFiles("Template/html/profile_edit.html")
	if tmplErr != nil {
...
func EditProfilePost(w http.ResponseWriter, r *http.Request, cookie *http.Cookie, user *structs.User) {
	...
	if password != "" {
		...
		if database.UpdatePass(user.ID, string(hashedPassword)) != nil {
