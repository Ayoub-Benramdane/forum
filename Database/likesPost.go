...
func CheckDislike(userID, postID int64) bool {
	var likes int64
	DB.QueryRow(`
        SELECT COUNT(*) FROM post_dislikes WHERE user_id = ? AND post_id = ?
    `, userID, postID).Scan(&likes)
	return likes > 0
}
...
func DeleteDislike(userID, postID int64) error {
	_, err := DB.Exec(`
        DELETE FROM post_dislikes WHERE user_id = ? AND post_id = ?
    `, userID, postID)
	return err
}
...
func CountDislikes(postID int64) (int64, error) {
	var likes int64
	err := DB.QueryRow(`
        SELECT COUNT(*) FROM post_dislikes WHERE post_id = ?
    `, postID).Scan(&likes)
	if err != nil {
		return 0, err
	}
